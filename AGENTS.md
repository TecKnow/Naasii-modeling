...
```
`uv sync --frozen` creates and populates `.venv` from `uv.lock`; rerun `uv sync` after dependency changes. After bootstrap, prefer the project `.venv/bin/...` tools for local work and automated tooling to avoid repeated `uv` cache-permission failures in restricted environments. In the Codex sandbox on WSL, `marimo check` appears to hang on this notebook, so use `pytest` and `py_compile` for routine validation there and run `marimo check` outside that sandbox when needed. Use the `uv run --with marimo[recommended] marimo ... --sandbox` forms only when you specifically need Marimo's isolated sandbox and `uv` can write to its cache.
